...
class TypescriptCodeGenerator implements CodeGenerator<Config> {
  readonly id = "skir-typescript-gen";
...
class TsModuleCodeGenerator {
  ...
  generate(): string {
    ...
    if (this.fileType === ".js") {
      const typesFileName = paths.basename(
        this.inModule.path.replace(/\.skir$/, ".d.ts"),
      );
      this.push(`/// <reference types="./${typesFileName}" />\n\n`);
    }

    this.push(`import * as $ from "${this.resolveClientModulePath()}";\n`);
    this.writeImports();
